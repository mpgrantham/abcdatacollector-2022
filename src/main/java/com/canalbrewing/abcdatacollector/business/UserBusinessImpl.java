...
import com.canalbrewing.abcdatacollector.dal.UserDao;
...
public class UserBusinessImpl implements UserBusiness {
	...
	@Value("${abcdatacollector.url}")
	private String abcdatacollectorUrl;

	@Value("${abcdatacollector.fromEmail}")
	private String fromEmail;

	@Value("${abcdatacollector.smtpServer}")
	private String smtpServer;

	@Value("${abcdatacollector.smtpPort}")
	private String smtpPort;

	@Value("${abcdatacollector.mailAccount}")
	private String mailAccount;

	@Value("${abcdatacollector.mailPassword}")
	private String mailPassword;

	@Value("${abcdatacollector.sessionTimeoutMillis}")
	private Long sessionTimeoutMillis;

	public UserSession getUserSession(String sessionToken) throws SQLException, AbcDataCollectorException {
		UserSession session = userDao.getUserSession(sessionToken);
		if (session == null) {
			throw new AbcDataCollectorException(AbcDataCollectorException.INVALID_SESSION,
					"Invalid Session. Session Not Found.");
		}

		if (session.getSessionActiveFl() == 0) {
			throw new AbcDataCollectorException(AbcDataCollectorException.INVALID_SESSION,
					"Invalid Session. Session Inactive.");
		}

		if ((session.getSessionDate().getTime() + sessionTimeoutMillis) < System.currentTimeMillis()) {
			throw new AbcDataCollectorException(AbcDataCollectorException.INVALID_SESSION,
	...
	public User signInUser(String username, String password, boolean staySignedIn)
		...
		if (user == null || User.STATUS_PENDING.equals(user.getStatus())) {
			throw new AbcDataCollectorException(AbcDataCollectorException.INVALID_LOGIN, MSG_INVALID_LOGIN);
		}
		...
		try {
			...
			throw new AbcDataCollectorException(AbcDataCollectorException.OTHER, ex.getMessage());
		}
	...
	public void signOutUser(UserSession session) throws SQLException {
	...
	public User signInUser(String signedInKey) throws SQLException, AbcDataCollectorException {
	...
	public User register(User user) throws SQLException, AbcDataCollectorException {

		if (userDao.getUserByUsername(user.getUserNm()) != null) {
			throw new AbcDataCollectorException(AbcDataCollectorException.DUPLICATE, MSG_USERNAME_EXISTS);
		}

		if (userDao.getUserByEmail(user.getEmail()) != null) {
			throw new AbcDataCollectorException(AbcDataCollectorException.DUPLICATE, MSG_EMAIL_EXISTS);
		}
		...
		body.append(abcdatacollectorUrl + "/confirmRegister/" + uuid);
	...
	public User confirmRegister(String verificationKey) throws SQLException, AbcDataCollectorException {

		Verification verification = getVerification(verificationKey);

		User user = userDao.getUserById(verification.getVerifiedUserId());
		if (user == null) {
			throw new AbcDataCollectorException(AbcDataCollectorException.NOT_FOUND, MSG_NO_USER_FOR_EMAIL);
		}
	...
	public User saveUser(User user) throws SQLException, AbcDataCollectorException {

		User u = userDao.getUserByUsername(user.getUserNm());
		if (u != null && u.getId() != user.getId()) {
			throw new AbcDataCollectorException(AbcDataCollectorException.DUPLICATE, "Username already exists.");
		}

		u = userDao.getUserByEmail(user.getEmail());
		if (u != null && u.getId() != user.getId()) {
			throw new AbcDataCollectorException(AbcDataCollectorException.DUPLICATE, "Email already exists.");
		}
	...
	public User updateUser(User user) throws SQLException, AbcDataCollectorException {
	...
	public User updateUsername(int userId, String userName) throws SQLException, AbcDataCollectorException {
	...
	public User resetPassword(String resetKey, String password) throws SQLException, AbcDataCollectorException {
	...
	public User getUserByResetKey(String resetKey) throws SQLException, AbcDataCollectorException {
		Verification verification = userDao.getVerification(resetKey);

		if (verification == null) {
			throw new AbcDataCollectorException(AbcDataCollectorException.NOT_FOUND, "Reset Link not found");
		}

		return userDao.getUserById(verification.getVerifiedUserId());
	}

	public void deleteUser(int userId) throws SQLException, AbcDataCollectorException {
		...
		if (found.isPresent()) {
			throw new AbcDataCollectorException(AbcDataCollectorException.INVALID_DELETE, "User is ADMIN for Observed");
		}
	...
	public Observed addObserved(int userId, String observedNm, String relationshipId)
		...
		if (found.isPresent()) {
			throw new AbcDataCollectorException(AbcDataCollectorException.DUPLICATE, "Observed name exists for User");
		}
	...
	public Observed grantAccess(String obsId, String email, String role, String relationshipId)
		...
		if (found != null && !found.isEmpty()) {
			...
			if (Observed.STATUS_ACTIVE.equals(obs.getAccessStatus())) {
				throw new AbcDataCollectorException(AbcDataCollectorException.DUPLICATE,
						"User already has access to this Observed.");
			}
		}
	...
	public String requestReassign(int userId, String observedId, String email)
			throws SQLException, AbcDataCollectorException {
		...
		verification.setRequestingUserId(userId);
		...
		body.append(abcdatacollectorUrl + "/reassign/" + uuid);
	...
	public Verification getReassignVerification(String verificationKey) throws SQLException, AbcDataCollectorException {
		Verification verification = userDao.getVerification(verificationKey);
		if (verification == null) {
			throw new AbcDataCollectorException(AbcDataCollectorException.NOT_FOUND, "Reassign Link not found");
		}
	...
	public String sendUsernameOrPassword(String email, String forgottenItem) throws AbcDataCollectorException {
		String message = "";
		...
		try {
			...
			if ("USERNAME".equals(forgottenItem)) {
				...
				body.append(abcdatacollectorUrl + "/reset/" + uuid);
	...
	private void sendEmail(String subject, StringBuilder body, String toEmail) throws AbcDataCollectorException {
		Properties props = System.getProperties();
		// -- Attaching to default Session, or we could start a new one --

		AbcDataCollectorAuthenticator authenticator = new AbcDataCollectorAuthenticator(mailAccount, mailPassword);
